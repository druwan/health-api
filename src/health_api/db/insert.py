...
def _insert_blood_glucose(
  ...
  rows = [
    (str(uuid4()), metric_id, metric_name, e.date, e.qty, e.meal_time, e.source) for e in entries
  ]
  cur.executemany(
    """
...
def _insert_insulin(
  ...
  rows = [
    (str(uuid4()), metric_id, metric_name, e.date, e.qty, e.reason, e.source) for e in entries
  ]
  cur.executemany(
    """
...
def insert_metrics(conn: psycopg.Connection, metrics: list[HealthMetric]) -> tuple[str, dict]:
  ...
  with conn.cursor() as cur:
    ...
    for metric in metrics:
      ...
      inserter, model_cls = METRIC_INSERTERS.get(metric.name, (_insert_quantity, QuantitySample))
      parsed = []
    ...
    for e in entries:
      try:
        ...
        if e.voltage_measurements:
          v_rows = [
            (str(uuid4()), ecg_id, v.date, v.voltage, v.units) for v in e.voltage_measurements
          ]
          cur.executemany(
            """
  ...
  with conn.cursor() as cur:
    ...
    for e in entries:
      try:
        ...
        if e.codings:
          coding_rows = [(str(uuid4()), med_id, c.code, c.system, c.version) for c in e.codings]
          cur.executemany(
            """
